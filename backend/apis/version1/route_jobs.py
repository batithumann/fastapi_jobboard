...
from db.repository.jobs import (create_new_job, get_job_by_id, get_active_jobs, update_job_by_id, delete_job_by_id)
...
def delete_job(id:int, db:Session=Depends(get_db), current_user: User=Depends(get_current_user_from_token)):
    ...
    if job.owner_id == current_user.id or current_user.is_superuser:
        delete_job_by_id(id, db, owner_id=current_user.id)
